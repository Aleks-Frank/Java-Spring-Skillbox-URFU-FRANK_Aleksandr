...
import com.example.rest_Api_posts_app.mapper.CategoryMapper;
...
import com.example.rest_Api_posts_app.web.response.category.CategoryListResponse;
    ...
    public ResponseEntity<CategoryListResponse> findAll(@RequestParam(defaultValue = "0") int pageNumber,
        ...
        return ResponseEntity.ok(
                CategoryListResponse.builder()
                        .categoryListResponse(categories.stream().map(categoryMapper::categoryFindAllToResponse).toList())
                        .build()
        );
    }
