...
import com.example.rest_Api_posts_app.mapper.UserMapper;
...
import com.example.rest_Api_posts_app.web.response.user.UsersListResponse;
...
public class UserController {
    ...
    @ApiResponse(
            responseCode = "200",
            content = {
                    @Content(schema = @Schema(implementation = UsersListResponse.class), mediaType = "application/json")
            }
    )
    @GetMapping
    public ResponseEntity<UsersListResponse> findAll(@RequestParam(defaultValue = "0") int pageNumber,
                                                    @RequestParam(defaultValue = "10") int pageSize) {
        Page<User> users = userService.findAll(pageNumber, pageSize);
        return ResponseEntity.ok(
                UsersListResponse.builder()
                        .users(users.stream().map(userMapper::userToResponse).toList()).build());
    }
